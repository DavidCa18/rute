...
export class LoginPage {
  ...
  inicioSesion(){
    ...
    this.api.post("verificacionUsuario", {email: this.usuario.email, password: this.usuario.password}).subscribe(
      (res: any) => {
       var resultado = res;
        if(resultado.length == 0){
          this.mostrarNotificacion('Correo Electrónico y/o Contrasena Incorrecta');
        }else{
          if(resultado[0].rol == 'CONDUCTOR'){
            this.mostrarNotificacion('Ingreso Correcto a la Plataforma');
            this.verificar.iniciarSesion(res[0]);
            this.navCtrl.setRoot(TabsPage);
          }else{
            this.mostrarNotificacion('Las credenciales que ingresó no pertenecen a un conductor');
          }
